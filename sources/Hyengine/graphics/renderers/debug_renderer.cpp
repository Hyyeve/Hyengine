...
namespace hyengine::graphics {
    ...
    debug_renderer::debug_renderer(): draw_count(0), write_index(0)
    {
    }
    ...
    void debug_renderer::allocate(const unsigned int memory_budget_mb)
    {
        if (is_allocated)
        {
    ...
    void debug_renderer::vertex(glm::vec3 pos, glm::vec4 color)
    {
        if (!is_allocated)
        {
    ...
    void debug_renderer::triangle(glm::vec3 a, glm::vec3 b, glm::vec3 c, glm::vec4 color)
    {
        vertex(a, color);
        vertex(b, color);
        vertex(c, color);
    ...
    void debug_renderer::quad(glm::vec3 a, glm::vec3 b, glm::vec3 c, glm::vec3 d, glm::vec4 color)
    {
        triangle(a, b, c, color);
        triangle(d, c, b, color);
    }

    void debug_renderer::rect(glm::vec2 a, glm::vec2 b, glm::vec4 color)
    {
        quad({a.x, a.y, 0}, {b.x, a.y, 0}, {a.x, b.y, 0}, {b.x, b.y, 0}, color);
    }

    void debug_renderer::line(glm::vec3 start, glm::vec3 end, glm::vec4 color, float thickness)
    {
    ...
    void debug_renderer::submit_geometry()
    {
    ...
    void debug_renderer::prepare_buffers()
    {
        vertex_buffer.increment_slice();
    }

    void debug_renderer::update_shader_uniforms(const float interpolation_delta, const graphics::camera& cam)
    {
    ...
    void debug_renderer::reload_shaders()
    {
        debug_shader.reload();
    }

    void debug_renderer::bind() const
    {
        vertex_format_buffer.bind_state();
        debug_shader.use();
    }

    void debug_renderer::draw() const
    {
