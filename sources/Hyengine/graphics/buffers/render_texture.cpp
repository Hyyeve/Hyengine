...
namespace hyengine::graphics {
    ...
    {
        if(buffer_id > 0) {
            logger::message_warn(logger::format("Attempted to initialize already initialized buffer!", " (buffer ", buffer_id, ")"), logger_tag);
            return;
        }
        ...
        glNamedRenderbufferStorageMultisample(buffer_id, internal_samples, internal_format, internal_size.x, internal_size.y);

        logger::message_info(logger::format("Allocated render texture ", buffer_id, ": ", internal_size.x, "x", internal_size.y, " ", logger::format_count(internal_samples, "MSAA sample")), logger_tag);
    }

    void render_texture::free()
    {
        glDeleteRenderbuffers(1, &buffer_id);
        logger::message_info(logger::format("Freed render texture ", buffer_id, "."), logger_tag);
        buffer_id = 0;
    }
    ...
    {
        glCopyImageSubData(source.get_id(), GL_RENDERBUFFER, 0, from_pos.x, from_pos.y, 0, buffer_id, GL_RENDERBUFFER, 0, to_pos.x, to_pos.y, 0, size.x, size.y, 0);
    }
