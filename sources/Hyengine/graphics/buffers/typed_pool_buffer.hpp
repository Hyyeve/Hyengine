...
namespace hyengine::graphics {
    ...
    public:
        ...
        ~typed_pool_buffer() { free(); }

        void allocate(const GLenum target, const GLsizeiptr elements) { internal_data_buffer.allocate(target, elements * sizeof(TYPE)); }
        void free() { internal_data_buffer.free(); }

        void shrink_staging_buffer() { internal_data_buffer.shrink_staging_buffer(); }

        bool try_allocate_space(const unsigned int elements, unsigned int& address) { return internal_data_buffer.try_allocate_space(elements * sizeof(TYPE), address); }
        void deallocate_space(const unsigned int address) { internal_data_buffer.deallocate_space(address); }
        [[nodiscard]] unsigned int get_last_allocated_element() const { return internal_data_buffer.get_last_allocated_address() / sizeof(TYPE); }
        ...
        void submit_uploads() { internal_data_buffer.submit_uploads(); }
        ...
        [[nodiscard]] GLsizeiptr get_elements() const { return internal_data_buffer.get_size() / sizeof(TYPE); }
        ...
        [[nodiscard]] unsigned int get_pending_upload_count() const { return internal_data_buffer.get_pending_upload_count(); }
